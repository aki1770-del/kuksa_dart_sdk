...
String renderTable({String root = '.'}) {
  ...
  for (final s in signalMap) {
    ...
    final inSet = kSnowSafetySignals.contains(s.path);
    b.writeln('| `${s.constant}` | `${s.path}` | ${spec.datatype} '
        '(${spec.type}) | ${_range(spec)} | ${inSet ? 'yes' : 'no'} |');
  }
  // The summary row said "all of the above" while the list held 10 of the 12
  // constants above it. Derive it from the constant so it cannot drift again.
  final documented = signalMap.map((s) => s.path).toSet();
  final excluded = [
    for (final s in signalMap)
      if (!kSnowSafetySignals.contains(s.path)) '`${s.constant}`',
  ];
  final extra = kSnowSafetySignals.where((p) => !documented.contains(p));
  if (extra.isNotEmpty) {
    throw StateError('kSnowSafetySignals contains ${extra.join(', ')}, which '
        'this table does not document. Add it to signalMap.');
  }
  b
    ..writeln('| `kSnowSafetySignals` | — | `List<String>` | '
        '${kSnowSafetySignals.length} of the ${signalMap.length} signals '
        'above as one subscription list${excluded.isEmpty ? '' : '; '
